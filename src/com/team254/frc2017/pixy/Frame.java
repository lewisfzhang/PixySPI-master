...
public class Frame {
	...
	public static class Block {
		...
		public void undistort() {
			...
			src.fromArray(new Point(centerX, centerY), new Point(centerX - width / 2, centerY - height / 2),
			...
			centerX = (int) Math.round(dst.toArray()[0].x);
			centerY = (int) Math.round(dst.toArray()[0].y);
			width = (int) Math.round(dst.toArray()[2].x - dst.toArray()[1].x);
			height = (int) Math.round(dst.toArray()[2].y - dst.toArray()[1].y);
		}
	}
